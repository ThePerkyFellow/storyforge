...
export default async function StoryTreePage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen pt-16 bg-animated">
      <div className="max-w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-4 mb-8 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-300 text-sm">Branch Tree</span>
          </div>
        </div>

        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <h1 className="font-story text-3xl sm:text-4xl font-bold text-white mb-2">
              {story.title}
            </h1>
          ...
          <div className="flex flex-wrap gap-3 mb-8">
            {branchLegend.map((b) => (
              <div
                key={b.name}
                className="flex items-center gap-2 px-3 py-1.5 glass rounded-full border border-white/10 text-sm"
              >
                <div
                  className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                  style={{ backgroundColor: b.color }}
                />
                <span className="text-slate-300 font-mono text-xs">{b.name}</span>
                {b.isCanon && (
                  <span className="text-amber-400 text-xs">(canon)</span>
                )}
                <span className="text-slate-500 text-xs">@{b.author}</span>
              </div>
            ))}
          </div>

          {/* Tree container */}
          <div className="glass rounded-2xl border border-white/10 overflow-hidden" style={{ minHeight: 500 }}>
            {treeNodes.length > 0 ? (
              <div className="p-6">
                <StoryBranchTree
                  nodes={treeNodes}
