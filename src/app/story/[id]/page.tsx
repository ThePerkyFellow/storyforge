        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mb-8">
              {story.genre && genreColors && (
                <span className={cn('tag mb-4', genreColors.bg, genreColors.text, 'border-transparent')}>
                  {story.genre}
                </span>
              )}
              <h1 className="font-story text-4xl sm:text-5xl font-bold text-white leading-tight mb-4">
                {story.title}
              </h1>
              {story.description && (
                <p className="text-slate-400 text-lg leading-relaxed mb-6">{story.description}</p>
              )}

              <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500">
                <span className="flex items-center gap-1.5">
                  <User className="w-4 h-4" />
                  <Link href={`/profile/${story.author?.username}`} className="text-amber-400 hover:text-amber-300 transition-colors">
              ...
              {story.tags && story.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {story.tags.map((tag: string) => (
                    <span key={tag} className="px-2.5 py-1 rounded-lg bg-white/5 text-slate-400 text-xs border border-white/8">
                      #{tag}
                    </span>
                  ))}
                </div>
            ...
            {canonBranch && (
              <div className="mb-10">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h2 className="font-story text-xl font-bold text-white">
                ...
                <div className="space-y-3">
                  {canonChapters.map((chapter: Chapter, idx: number) => (
                    <Link
                      ...
                      className="flex items-center justify-between glass rounded-xl p-4 border border-white/8 card-hover group"
                    >
            ...
            {forkBranches.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <span className="px-2 py-0.5 rounded text-xs bg-violet-500/15 text-violet-400 border border-violet-500/25">
                ...
                <div className="space-y-3">
                  {forkBranches.map((branch: Branch) => {
                    ...
                    return (
                      <div key={branch.id} className="glass rounded-xl p-5 border border-violet-500/15 card-hover">
                        <div className="flex items-start justify-between gap-4 mb-3">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <GitBranch className="w-3.5 h-3.5 text-violet-400" />
                              <span className="text-violet-300 text-sm font-medium font-mono">
                                {branch.name}
                              </span>
                            </div>
                            {branch.description && (
          ...
          <div className="space-y-5">
            ...
            <div className="glass rounded-2xl p-5 border border-white/10">
              <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Story Stats</h3>
            ...
            {story.author && (
              <div className="glass rounded-2xl p-5 border border-white/10">
                <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Author</h3>
                <Link href={`/profile/${story.author.username}`} className="flex items-center gap-3 group">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-amber-500 to-violet-600 flex items-center justify-center flex-shrink-0">
                  ...
                  <div>
                    ...
                    <div className="text-slate-500 text-xs">@{story.author.username}</div>
                  </div>
                </Link>
                {story.author.bio && (
                  <p className="text-slate-400 text-xs leading-relaxed mt-3">{story.author.bio}</p>
                )}
              </div>
            )}
            ...
            <div className="glass rounded-2xl p-5 border border-violet-500/20 bg-violet-500/5">
              <div className="flex items-center gap-2 mb-3">
                <GitBranch className="w-4 h-4 text-violet-400" />
                <h3 className="text-sm font-semibold text-white">Fork this story</h3>
              </div>
              ...
              <Link
                href={`/story/${id}/tree`}
                className="btn-fork w-full justify-center text-sm"
